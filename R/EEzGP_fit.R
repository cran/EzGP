#' @title The Fitting Function of \code{EEzGP} Model
#'
#' @description Fits an Efficient Easy-to-Interpret Gaussian process (EEzGP) model to a dataset as described in \code{reference 1}.
...
EEzGP_fit <- function(X, Y, p, q, m, tau = 0, lb = "T", ub = "T",  x0 = "T",
  ...
  eval_f_list <- function(parv) {
    ...
    ### derivative for correlation parameter theta_(h)_l for h=1,...,q, l=2,...,m_h,
    grad_cor <- function(h,l)
    {
      ### derative function of theta_0_s for wi, wj
    ...
    for (h0 in 1:q)
    {
      for (l0 in 2:m[h0])
      {
        grad_res = c(grad_res, grad_cor(h0,l0))
      }
    }
